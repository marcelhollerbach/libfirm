...
handle_cmp(ir_node *node, bool *const changed)
{
	...
	*changed = true;
	...
	//FIXME backedges
	if (get_irn_n_outs(res.conv) == 1) {
		exchange(res.conv, res.pre_conv);
	} else {

		//Full exchange is not clearly possible,
		//as the conv is not only required by the cmp
		//so we just change the in of the Cmp, and
