	...
	if (info->valid){
		switch (get_irn_opcode(node)) {
			case iro_Const: {
				...
				int mode_sign_bits = 0;

				if (mode_is_signed(mode))
					mode_sign_bits = 1;

				if (value > 0)
				  required_bits = log2_floor(value) + 1 + mode_sign_bits;
				else if (value < 0)
					required_bits = log2_floor(-value) + 1 + mode_sign_bits;
				else
					required_bits = 1;
...
evalulate_node(ir_node *node, pqueue_t *queue)
{
	...
	switch(get_irn_opcode(node)) {
		...
		case iro_Conv: {
			...
			if (!mode_is_signed(old_mode) && mode_is_signed(mode) &&
				  op_bitwidth->stable_digits == 0) {

				  if (new.stable_digits > 0)
						new.stable_digits -= 1; //we have to safe one bit for indicating the sign
			}
		...
		case iro_Shr: { // the worst case is the maximum number
			...
			if (mode_is_signed(mode_a) && min_val == 0) {
